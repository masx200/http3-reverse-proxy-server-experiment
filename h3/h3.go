...
func DoHTTP3Client(msg *dns.Msg, dohServer string) (r *dns.Msg, err error) {
	client := &http.Client{
	...
	//http request doh
	res, err := client.Post(dohServer, "application/dns-message", strings.NewReader(string(body)))
	if err != nil {
		log.Println(err)
		return nil, err
	}
